...
description: Use when designing, adding or changing a ship blueprint in the who-knows Godot project - a new ship, a second ship in a scene, a shipyard or generator emitting a grid, or moving the helm, canopy, airlock, engines, reactors or rooms of an existing ship. Also when a ship won't launch, won't turn or brake, pitches under burn, browns out, or strands the player (stuck by the chair, an airlock that never cycles).
---
...
8. **Probe the real scene:** run `ship_probe.gd` (in this folder) **without** `--headless`. It
   prints the validator, the stats, rooms, pods and airlocks, and fps. It also sits, stands and
   walks, and flags `STUCK`.
9. **Render and show the owner:** eye-height (1.6 m) views of the bridge, the seated view, the
   corridor, each room and the exterior. Cycle the airlock both ways, look out of the windows, and
   hold ≥120 fps at 1280×720.
